...
const cronJobGame1p = (io) => {
  cron.schedule("*/30 * * * * *", async () => {
    await winGoController.addWinGo(30);
    await winGoController.handlingWinGo1P(30);
    const [wingo30] = await connection.execute(
      'SELECT * FROM `wingo` WHERE `game` = "wingo30" ORDER BY `id` DESC LIMIT 2 ',
      [],
    );
    const data = wingo30;
