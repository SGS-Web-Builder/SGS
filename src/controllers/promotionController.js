  ...
  try {
    ...
    const claimedRewardsData = claimedRewardsRow.map((claimedReward) => {
      ...
      return {
        ...
        amount: claimedReward.amount,
        status: claimedReward.status,
        time: moment(Number(claimedReward.time)).format(
...
const getCommissionDetailsByDate = async (req, res) => {
  try {
    const authToken = req.cookies.auth;
    const date = req.query?.date;
    const [userRow] = await connection.execute(
      "SELECT `phone`, `code` FROM `users` WHERE `token` = ? AND `veri` = 1",
      [authToken],
    );
    const user = userRow?.[0];
    ...
    if (startOfDay === "Invalid date" || endOfDay === "Invalid date") {
      return res.status(400).json({
        status: false,
    ...
    const [commission] = await connection.execute(
      "SELECT SUM(money) AS commission_payout, SUM(bet_money) AS betting_amount, COUNT(DISTINCT from_user_phone) AS bettors_count FROM `commissions` WHERE `phone` = ? AND `time` >= ? AND `time` <= ?",
      [user.phone, startOfDay, endOfDay],
    );

    const bettingAmount = Number(commission[0]?.betting_amount || 0);
    const commissionPayout = Number(commission[0]?.commission_payout || 0);
    const bettorsCount = Number(commission[0]?.bettors_count || 0);

    if (bettorsCount === 0) {
      return res.status(200).json({
        data: {
          date: moment(startOfDay).format("YYYY-MM-DD HH:mm:ss"),
          settlementDate: moment(endOfDay).format("YYYY-MM-DD HH:mm:ss"),
          bettingAmount: 0,
          commissionPayout: 0,
          bettorsCount: 0,
        },
        status: true,
        message: "Successfully fetched commission details",
      });
    }
...
const getCurrentRebateInfo = async (req, res) => {
  try {
    const authToken = req.cookies.auth;
    const date = req.query?.date;
    const [userRow] = await connection.execute(
    ...
    const endOfTodayTimestamp = moment().endOf("day").valueOf();
    ...
    const gameTrxBettingAmount = gameTrx[0].totalBettingAmount || 0;

    const bettingAmount =
      parseInt(gameWingoBettingAmount) +
      parseInt(gameK3BettingAmount) +
      parseInt(game5DBettingAmount) +
      parseInt(gameTrxBettingAmount);

    const [rebate_commissions] = await connection.query(
      "SELECT SUM(money) as totalRebateCommissions FROM rebate_commissions WHERE phone = ? AND `time` >= ? AND `time` <= ?",
      [phone, startOfTodayTimestamp, endOfTodayTimestamp],
    );
    const [rebate_commissions_forBet] = await connection.query(
      "SELECT SUM(bet_money) as totalPreviousBetAmount FROM rebate_commissions WHERE phone = ? AND `time` >= ? AND `time` <= ?",
      [phone, startOfTodayTimestamp, endOfTodayTimestamp],
    );
    const todayPreviousBetAmount =
      rebate_commissions_forBet?.[0]?.totalPreviousBetAmount;
    const todayRebateCommission =
      rebate_commissions?.[0]?.totalRebateCommissions;

    const [rebate_commissions_total] = await connection.query(
      "SELECT SUM(money) as totalRebateCommissions FROM rebate_commissions WHERE phone = ?",
      [phone],
    );
    const totalRebateCommission =
      rebate_commissions_total?.[0]?.totalRebateCommissions;

    const [rebate_commissions_list] = await connection.query(
      "SELECT * FROM rebate_commissions WHERE phone = ?",
      [phone],
    );
    const rebateCommissionList = rebate_commissions_list;

    return res.status(200).json({
      data: {
        bettingAmount:
          bettingAmount - todayPreviousBetAmount < 0
            ? 0
            : bettingAmount - todayPreviousBetAmount,
        todayRebateCommission: todayRebateCommission || 0,
        totalRebateCommission: totalRebateCommission || 0,
        rebateCommissionList,
      },
      status: true,
      message: "Successfully fetched commission details",
    });
...
const oneClickRebateClaim = async (req, res) => {
  try {
    const authToken = req.cookies.auth;
    const date = req.query?.date;
    const [userRow] = await connection.execute(
    ...
    const leftBettingAmount =
      bettingAmount - todayPreviousBetAmount < 0
        ? 0
        : bettingAmount - todayPreviousBetAmount;

    if (leftBettingAmount < 0) {
      return res.status(400).json({
        status: false,
        message: "You have no betting amount to claim rebate commissions",
      });
    }

    const rebateCommissionsAmount = leftBettingAmount * 0.0005;
    const commissionId = generateCommissionId();

    await connection.query(
      "INSERT INTO rebate_commissions (commission_id, phone, game_type, rate, money, bet_money, time, is_settled) VALUES ?",
      [
        [
          [
            commissionId,
    ...
    await connection.query(
      "UPDATE users SET money = money + ?, total_money = total_money + ? WHERE `phone` = ?",
      [rebateCommissionsAmount, rebateCommissionsAmount, phone],
    );

    return res.status(200).json({
      status: false,
      message: "Rebate commissions settled successfully!",
    });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
...
const promotionController = {
  ...
  getCommissionDetailsByDate,
  oneClickRebateClaim,
  getCurrentRebateInfo,
};
