...
import { WITHDRAWAL_METHODS_MAP } from "./withdrawalController.js";
...
const userInfo = async (req, res) => {
  ...
  const [withdraw_inr] = await connection.query(
    "SELECT SUM(money) as total_money  FROM withdraw WHERE `phone` = ? AND status = 1 AND tp = ?",
    [rows[0].phone, WITHDRAWAL_METHODS_MAP.BANK_CARD],
  );
  let totalWithdrawINR = withdraw_inr[0].total_money || 0;

  const [withdraw_usdt] = await connection.query(
    "SELECT SUM(money) as total_money FROM withdraw WHERE `phone` = ? AND status = 1 AND tp = ?",
    [rows[0].phone, WITHDRAWAL_METHODS_MAP.USDT_ADDRESS],
  );
  let totalWithdrawUSDT = withdraw_usdt[0].total_money || 0;

  const totalWithdraw =
    totalWithdrawINR +
    totalWithdrawUSDT * Number(process.env.USDT_INR_EXCHANGE_RATE);
...
const infoUserBank = async (req, res) => {
  ...
  try {
    ...
    const [aviatorBetMoneyResult] = await connection.query(
      `SELECT SUM(bet_amount) AS aviatorBetMoney FROM jdb_bets WHERE gType = 22 AND mType = 22001 AND phone = '${userInfo.phone}' ${betTimeInterval}`,
    );
    const aviatorBetMoney =
      Number(aviatorBetMoneyResult[0].aviatorBetMoney) || 0;
