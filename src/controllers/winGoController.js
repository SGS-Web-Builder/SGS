  ...
  try {
    ...
    if (commissionsToInsert.length > 0) {
      await connection.query(
        "INSERT INTO commissions (commission_id, phone, from_user_phone, money, bet_money, level, time) VALUES ?",
        [commissionsToInsert],
      );
    }
...
const betWinGo = async (req, res) => {
  ...
  if (VALID_TYPE_IDS.indexOf(Number(typeid)) === -1) {
...
const addWinGo = async (game) => {
  try {
    ...
    if (isPendingGame) {
      ...
      let adminWingoKey = "";
      if (game == 30) adminWingoKey = "wingo30";
