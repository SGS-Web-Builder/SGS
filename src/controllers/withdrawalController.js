...
const approveAndInitiateAquapayWithdrawalRequest = async (req, res) => {
  ...
  try {
    ...
    const response = await axios({
      method: "post",
      url: `${process.env.AQUAPAY_API_URL}/api/withdraw/rspay/initiate`,
      data: {
        ...
        api_key: process.env.AQUAPAY_RSPAY_API_KEY,
        password: password,
      },
    });
    ...
    await connection.execute(
      `UPDATE withdraw SET status = 3, remarks = ?, order_id = ? WHERE id = ?`,
