...
function getNthTermOfPeriodSinceStart(seconds) {
  if (isNaN(seconds)) {
...
export function generatePeriod(gameRepresentationId, minutes) {
  ...
  const nthPeriodSinceDayStart = getNthTermOfPeriodSinceStart(seconds);

  let paddedNthPeriodSinceDayStart = _.padStart(
    nthPeriodSinceDayStart.toString(),
    4,
    "0",
  );

  let NewGamePeriod = `${TodayPeriod}${gameRepresentationId}${paddedNthPeriodSinceDayStart}`;
