...
$(
  `.container-fluid:eq(1) .row:eq(0) .info-box-content:eq(${POSITION_MAP[typeid]}) .info-box-text`,
).css("color", "#e67e22");
...
socket.on("data-server", function (msg) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (typeid == "5")
        is = $("#ketQua").text(
          `Next Result: ${response.setting[0].wingo30 == "-1" ? "Random" : response.setting[0].wingo30}`,
        );
      ...
      if (typeid == "5")
        $("#winrate").text(
          `Next Result: ${response.setting[0].bs30 == "-1" ? "Random" : response.setting[0].bs30}`,
        );
    },
  });
});
